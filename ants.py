...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        """
        # BEGIN Problem 3 and 4  # REPLACE THIS LINE
        new_place = self.place
        ab = []
        trans = 0
        while not ab and not new_place.is_hive:
            if (self.lower_bound <= trans <= self.upper_bound):
                ab = new_place.bees
            new_place = new_place.entrance
            trans += 1
        return random_bee(ab)
        # END Problem 3 and 4
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        queen_exit = self.place.exit
        while queen_exit:
            if queen_exit.ant:
                if not queen_exit.ant.isDoubled:
                    queen_exit.ant.double()
                if queen_exit.ant.is_container and queen_exit.ant.ant_contained:
                    if not queen_exit.ant.ant_contained.isDoubled:
                        queen_exit.ant.ant_contained.double()
            queen_exit = queen_exit.exit
        # END Problem 12
