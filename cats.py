...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def return_func(paragraph): 
        no_punc = remove_punctuation(paragraph)
        lowercased = lower(no_punc)
        splitted = split(lowercased)
        for words in splitted: 
            for check in subject: 
                if words == check: 
                    return True
        return False
    return return_func
...
def accuracy(typed, source):
    ...
    if len(source_words) > len(typed_words): 
        ...
        return (counter * 100 / len(source_words)) * (1-(difference / len(typed_words)))
    else: 
        for i in range(len(source_words)):
            if typed_words[i] == source_words[i]: 
                counter += 1
    return counter * 100 / len(source_words)
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    word_calculated = len(typed) / 5 
    time_in_min = 60 / elapsed 
    return word_calculated * time_in_min
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list: 
        if word == typed_word: 
            ...
            if diff_function(typed_word, word, limit) < cur: 
                cur = diff_function(typed_word, word, limit)
                cur_word = word
    if cur > limit: 
        return typed_word
    else: 
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helped(change, click, origin): 
        if (change > limit): 
            return limit + 1
        if (not click and not origin): 
            return change
        if (not click or not origin): 
            return helped(change + 1, click[1: ], origin[1: ])
        if (click[0] == origin[0]): 
            return helped(change, click[1:], origin[1:])
        return helped(change + 1, click[1:], origin[1:])
    return helped(0, typed, source)
    # END PROBLEM 6
...
def report_progress(typed, prompt, user_id, upload):
    ...
    while (i < len(typed)) and typed[i] == prompt[i]: 
        cur += 1
        i += 1
    upload({'id': user_id, 'progress': (cur/len(prompt))})
...
def time_per_word(words, times_per_player):
    ...
    for a in times_per_player: 
        emp_list.append([a[i + 1] - a[i] for i in range(len(a) - 1)])
    return match(words, emp_list)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    last = [[] for x in player_indices]
    for word in word_indices:
        player, winner = 0, time(match, player_indices[0], word)
        for emp_list in player_indices:
            if time(match, emp_list, word) < winner:
                player, winner = emp_list, time(match, emp_list, word)
        last[player].append(get_word(match, word))
    return last
