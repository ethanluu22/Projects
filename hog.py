...
def perfect_square(x): 
    ...
    if unsquared == x: 
        return True
    while unsquared != x: 
        if unsquared * unsquared == x: 
            return True
        unsquared += 1
    return False
def next_perfect_square(z): 
    next_z = z + 1
    while perfect_square(next_z) != True: 
        next_z += 1 
    return next_z
# END PROBLEM 4
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while a < goal: 
        while i < goal: 
            if strategy(a, i) != strategy(a, i + 1):
                return False
            i += 1 
        a += 1
        i = 0
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while i < 10:
        ...
        check = averaged_dice(incremented, dice)
        if check > store:
             store = check
             a = incremented
        i += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    if square_update(0, score, opponent_score) >= threshold + score: 
        return 0
    else: 
        return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
